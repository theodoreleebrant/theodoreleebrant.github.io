...
# installfest
sudo apt install -y build-essential vim curl wget git htop tmux tree jq unzip \
	zip rsync net-tools dnsutils traceroute tcpdump nmap lsof ripgrep ncdu \
	btop texlive-full sublime-text sublime-merge flatpak python3-smbc tlp \
	gnome-software-plugin-flatpak gnome-shell-extension-manager smbclient gimp
...
flatpak remote-add --if-not-exists flathub \
	...
	&& flatpak install -y flathub io.github.focustimerhq.FocusTimer
curl -fsSL https://tailscale.com/install.sh | sh
curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y

# get things up
. "$HOME/.cargo/env" && sudo systemctl enable --now tlp && sudo tailscale up

# sanity check
git --version; vim --version | head -n 1; code --version | head -n 1; \
	tailscale version; rustc --version; cargo --version
```
